...
try:
    ...
    try:
        ...
        base = "yolo11n.pt"
    ep = 220 if DEV == 0 else 40
    m = YOLO(base)
    m.train(data="/kaggle/working/data.yaml", epochs=ep, imgsz=1024,
            batch=(14 if DEV == 0 else 4), device=DEV, workers=4,
            ...
            project="/kaggle/working/runs", name="v33", exist_ok=True)
    best = "/kaggle/working/runs/v33/weights/best.pt"
    mm = YOLO(best)
    ...
    shutil.copy(onnx, "/kaggle/working/manga_panel_detector_v33_1024.onnx")
    shutil.copy(best, "/kaggle/working/best_v33.pt")
